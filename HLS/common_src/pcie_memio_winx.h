...
class fpga_xDMA_winX {
    ...
    public :
        ...
        int fpga_poke_fixed_N(uint32_t AXI_addr, char *buffer, uint32_t size ) {
            bool Exit_f_condition = false;
            LARGE_INTEGER AXI_ADDRESS;
            // set file pointer to offset of target address within PCIe BAR
            XDMA_KEYHOLE_DATA kholeData;
            kholeData.offset = (ULONG)AXI_addr;
            kholeData.ptrAddr = (PULONG)buffer; 
            kholeData.size = (ULONG)size;
            BOOL success = DeviceIoControl(device_user, IOCTL_WRITE_KEYHOLE_REGISTER, &kholeData, sizeof(XDMA_KEYHOLE_DATA), nullptr, 0, &Number_of_bytes_written, NULL);
            if (!success) {
                fprintf(stderr, "Error with device ioctl, win32 error code: %ld\n", GetLastError());
                Exit_f_condition = true;
            }
            return ((Exit_f_condition) ? -1 : 0);
        }
        ...
        int fpga_poke_N(uint32_t AXI_addr, char *buffer, uint32_t size ) {
            int64_t current_xfer_size;
            bool Exit_f_condition = false;
            LARGE_INTEGER AXI_ADDRESS;
            // set file pointer to offset of target address within PCIe BAR
        ...
        int fpga_poke(uint32_t AXI_addr, uint32_t data_val ) {
            bool Exit_f_condition = false;
        ...
        int fpga_peek_N(uint32_t AXI_addr, char *buffer, uint32_t size ) {
            int64_t current_xfer_size;
            bool Exit_f_condition = false;
            LARGE_INTEGER AXI_ADDRESS;
            current_xfer_size = size;
