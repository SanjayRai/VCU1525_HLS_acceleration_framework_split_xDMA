...
void fpga_PCIE_BANDWIDTH_test64 (fpga_xDMA_winX *my_fpga_xDMA_ptr, uint64_t AXI_ADDRESS,  char *A_IN, uint32_t xfer_size) {
...
void fpga_test_AXIL_LITE_8KSCRATCHPAD_BRAM (fpga_xDMA_winX *fpga_AXI_Lite_ptr) {
...
void  fpga_read_temprature (fpga_xDMA_winX *fpga_AXI_Lite_ptr, SysMon_temp_struct *sys_temp, int average_num) {
...
uint32_t  fpga_read_status (fpga_xDMA_winX *fpga_AXI_Lite_ptr) {
...
void fpga_PROGRAM_NORTH_PR (fpga_xDMA_winX *fpga_AXI_Lite_ptr, string PR_binFile_name) {
    ...
    ICAP_prog_PR_binfile_fast (fpga_AXI_Lite_ptr, PR_binFile_name);
...
void fpga_xfer_data_to_card64(fpga_xDMA_winX *my_fpga_xDMA_ptr, uint64_t AXI_ADDRESS, char *data_buf_ptr, uint32_t XFER_SIZE) {
...
void  fpga_run_kernel(fpga_xDMA_winX *fpga_AXI_Lite_ptr) {
...
void  fpga_set_kernel_arguments_32(fpga_xDMA_winX *fpga_AXI_Lite_ptr, uint32_t Addr_offset, uint32_t data) {
...
void  fpga_set_kernel_arguments_64(fpga_xDMA_winX *fpga_AXI_Lite_ptr, uint32_t Addr_offset, uint64_t data) {
...
void  fpga_run_NORTH_PR64(fpga_xDMA_winX *fpga_AXI_Lite_ptr, uint64_t AXI_ADDRESS_in0, uint64_t AXI_ADDRESS_RESULTS, uint32_t NUM_OF_DATA_SETS) {
...
void fpga_get_Kernel_execution_time (fpga_xDMA_winX *fpga_AXI_Lite_ptr, double HW_Kernel_frequency, kernel_execution_metric_struct *kernel_execution_metric) {
...
int  fpga_check_compute_done_NORTH_PR (fpga_xDMA_winX *fpga_AXI_Lite_ptr) {
...
void fpga_xfer_data_from_card64(fpga_xDMA_winX *my_fpga_xDMA_ptr, uint64_t AXI_ADDRESS_RESULTS, char *data_buf_ptr, uint32_t XFER_SIZE) {
/* Read the PR_HLS Control register to poll the DOne bit (bit 1) */ 
...
// ------------ Clean -----------------------
int fpga_clean (fpga_xDMA_winX *my_fpga_xDMA_ptr) {
        my_fpga_xDMA_ptr->fpga_xDMA_clean();

        return 0;
}

void ICAP_prog_PR_binfile (fpga_xDMA_winX *fpga_AXI_Lite_ptr, string PR_binFile_name) {
...
void xDMA_througput_h2c64 (fpga_xDMA_winX *my_fpga_xDMA_ptr, uint64_t AXI_ADDRESS,  char *data_buffer, uint32_t xfer_size) {
...
void xDMA_througput_c2h64 (fpga_xDMA_winX *my_fpga_xDMA_ptr, uint64_t AXI_ADDRESS, char *data_buffer, uint32_t xfer_size) {
    auto start_t = chrono::high_resolution_clock::now();
...
void fpga_PROGRAM_PR_CLOCK (fpga_xDMA_winX *fpga_AXI_Lite_ptr, float PR_Frequency) {
...
void ICAP_prog_PR_binfile_fast (fpga_xDMA_winX *fpga_AXI_Lite_ptr, string PR_binFile_name) {
    ...
    long long file_size_words;
    char *bitStream_buffer;
    uint32_t *bitStream_buffer_byteSwapped;
    uint32_t *bitstream_ptr;
...
/* Program Partial Bit file */
    ...
    file_size_words = (file_size/4); 
    fpga_bin_file.seekg(0, fpga_bin_file.beg);
    cout << "PR Bitfile size =  " << file_size << " Bytes (" << file_size_words << ") 32Bit Words\n" ;
    bitStream_buffer = new char [file_size];
    bitStream_buffer_byteSwapped = new uint32_t [file_size_words];
    bitstream_ptr = (uint32_t *)bitStream_buffer;
    fpga_bin_file.read(bitStream_buffer, file_size);
    fpga_bin_file.close();
    
    //srai_dbg_file.open("Srai_DBG_VEC_MULT_BIN.TXT");
    // Reset the ICAP
    fpga_AXI_Lite_ptr->fpga_poke(ICAP_CNTRL_REG, 0xC); 
    fpga_AXI_Lite_ptr->fpga_poke(ICAP_CNTRL_REG, 0x0); 
    // Check if the ICAP is ready
    ...
    for (int i = 0 ; i < file_size_words; i++) {
        byte_swapped = ((*bitstream_ptr>>24)&0x000000ff) | \
                       ...
                       ((*bitstream_ptr<<24)&0xff000000);
        bitstream_ptr++;
        bitStream_buffer_byteSwapped[i] = byte_swapped;
        //srai_dbg_file << dec << i << hex << " : " << setfill('0') << bitStream_buffer_byteSwapped[i] << endl;
    }
   
    string srai_dbg;
    uint32_t idx = 0;
    uint32_t dbg_count = 0;
    const uint32_t ICAP_FIFO_DEPTH = 60;
    while (idx < (file_size_words - ICAP_FIFO_DEPTH)) {
         if ( 0) {
             cout << "Enter a key to continue";
             cin >> srai_dbg;
         }
        fpga_AXI_Lite_ptr->fpga_poke_fixed_N(ICAP_WR_FIFO_REG, (char*)&bitStream_buffer_byteSwapped[idx], ICAP_FIFO_DEPTH); 
        fpga_AXI_Lite_ptr->fpga_poke(ICAP_CNTRL_REG, 0x1); 
        itn_count = 0;
        // Wait till Fifo is drained
        while (ret_data != 0x3F) {
    ...
    cout << "Srai DBG :: idx  " << idx << "  trunc = "  << (file_size_words -idx) << " FS = " << file_size_words <<  " Dbg _count  = 0x" << hex << dbg_count << dec << endl;
    fpga_AXI_Lite_ptr->fpga_poke_fixed_N(ICAP_WR_FIFO_REG, (char *)&bitStream_buffer_byteSwapped[idx], (file_size_words -idx)); 
    fpga_AXI_Lite_ptr->fpga_poke(ICAP_CNTRL_REG, 0x1); 
    itn_count = 0;
    // Wait till Fifo is drained
    while (ret_data != 0x3F) {
   ...
   //srai_dbg_file.close();
   delete [] bitStream_buffer;
   delete [] bitStream_buffer_byteSwapped;
}
